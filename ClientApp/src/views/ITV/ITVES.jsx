...
import NewITVModal from "./NewITVModal";
...
const ITVES = () => {
  ...
  const handleSubmit = (ITV) => {
    const auth = { Authorization: `bearer ${localStorage["token"]}` };
    ITV.date = new Date();
    

    if (ITV.id === 0) {
      Services.post(Config.apiUrl + "/itv", ITV, { headers: auth })
        .then(({ data }) => {
      ...
      Services.put(Config.apiUrl + "/itv/" + ITV.id, ITV, {
        headers: auth,
      })
        .then(({ data }) => {
    ...
    <>
      ...
      <Card>
        <Card.Header>
          ...
          >
            <FontAwesome name="fas fa-plus-circle" /> New ITV
          </Button>
          <Card.Title>ITVES</Card.Title>
        </Card.Header>
        <Card.Body>
          <MDBDataTableV5
            hover
